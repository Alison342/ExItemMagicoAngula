...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, CommonModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatIconModule, LucideAngularModule],
...
export class CardItemMagicoComponent {
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(DialogItemMagicoComponent, {
      data: { nome: this.item.nome, tipo: this.item.tipo, poder: this.item.poder, imagemUrl: this.item.imagemUrl },
    });
